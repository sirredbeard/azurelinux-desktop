...
- **Microsoft Copilot Flatpak:** system-wide OSTree (`org.gnome.Platform//50` + `com.github.sirredbeard.copilot-desktop-gtk` + Pages remote). **Do not `flatpak install` into Anaconda `/mnt/sysimage` during `%post --nochroot`** — that hung 90+ minutes on GHA (run 30899051688) while the same pull finished in ~30s on the build host and in kiwi `config.sh`. Pattern: `scripts/prestage-copilot-flatpak-system.sh` fills `/workspace/prestage/flatpak-system` (live/disk) or `assets/flatpak-system-latest` (installer assets.tar) **before** livemedia/kiwi; kickstart/ks only `cp -a` into the target `/var/lib/flatpak`.
