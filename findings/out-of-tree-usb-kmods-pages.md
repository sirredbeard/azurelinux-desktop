...
- Secrets: `GPG_PRIVATE_KEY` / `PUBLIC_KEY` / `KEY_ID` on this repo
...
(key id `8DA5774C35DA9BF9`). Secrets: `GPG_PRIVATE_KEY` /
`GPG_PUBLIC_KEY` / `GPG_KEY_ID`. Public in
`packaging/gpg/public.asc` and
`assets/pki/rpm-gpg/RPM-GPG-KEY-azurelinux-desktop`.
fingerprint is what DNF checks. See `packaging/gpg/README.md`.
