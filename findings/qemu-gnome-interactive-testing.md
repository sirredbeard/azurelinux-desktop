...
(screendump + sendkey + mouse_move) on a Fedora host. Accumulated 2026-07-22/23/24/25.
...
## VNC vs GTK display backend (2026-07-24/25)

**VNC (`:display vnc=127.0.0.1:N`) is the only reliable capture path** for scripted
testing. GTK display (`-display gtk`) works for manual visual inspection but has no
programmatic capture mechanism; `screendump` captures the legacy VGA framebuffer, not
the GTK window content, so it's the same all-black-after-Wayland problem.

**GTK window vs QEMU monitor mouse input:** QEMU GTK frontend and SDL frontend both have
Wayland pointer-grab behavior that is unreliable for automated testing. The GTK window
grabs the pointer when clicked and releases on Escape or Middle-click, but these grab
semantics are host-compositor-dependent (Wayland on the host means QEMU can't grab the
pointer at all without the host compositor cooperating). Use `usb-tablet` with VNC for
reliable absolute-coordinate mouse input.

**OVMF variables file (installer ISO testing):** `scripts/qemu-test-install-iso.sh`
previously sourced the OVMF helper but never attached the firmware. Result: QEMU booted
BIOS while the kickstart requested an EFI partition. Anaconda correctly rejected the
contradiction. Fix: QEMU helpers must attach both OVMF_CODE.fd (read-only) and a
per-test writable `OVMF_VARS.fd` copy. Always confirm EFI boot by checking for the
BdsDxe message in the serial log before treating an Anaconda layout error as a
kickstart bug.

**BdsDxe firmware text before GRUB:** Brief UEFI firmware text (`Loading...`,
`Starting...`) visible before GRUB menu renders in QEMU GTK window. QEMU-specific OVMF
behavior; not present on real hardware with a graphical UEFI. Not actionable — expected
in QEMU.
...
For testing without modifying the source artifact, pass `-snapshot` to QEMU:

```bash
qemu-system-x86_64 ... -drive file="$QCOW2",format=qcow2,snapshot=on ...
```

With `-snapshot`, all writes go to a temporary overlay discarded on exit. The source
qcow2 is not modified. This is the standard pattern in `scripts/qemu-test-disk-image.sh`
and `scripts/qemu-test-live-iso.sh` — never modify the downloaded artifact directly.

For the installer ISO, pass a **fresh** (empty) writable target disk alongside the
read-only ISO:

```bash
qemu-img create -f qcow2 "$TARGET_DISK" 64G
qemu-system-x86_64 \
  -drive file="$INSTALLER_ISO",media=cdrom,readonly=on,if=ide \
  -drive file="$TARGET_DISK",format=qcow2,if=virtio \
  ...
```

After installation completes, reboot without the ISO attached to test the installed
target. Use a separate `OVMF_VARS.fd` copy per test run so NVRAM entries from one
install don't persist into the next.

---

## App-specific notes added 2026-07-25

**GitHub Desktop:** process starts and creates a window (visible as a dark gray
rectangle in QEMU). Window content does **not** render under QEMU virtio-vga/Wayland —
Electron renderer produces a blank canvas. QEMU test environment limitation only;
GitHub Desktop renders correctly on real hardware and non-QEMU VMs. Confirm binary
presence and process launch; skip visual content verification in QEMU.

**EFI boot "noise" (QEMU-specific):** BdsDxe firmware text (loading/starting UEFI
DVD-ROM) briefly visible before GRUB takes over when booting from virtual ATAPI CDROM.
Not present on real hardware with a graphical UEFI. Not actionable.

**Installer ISO boot timing:** squashfs decompression from a virtual ATAPI CDROM takes
15–50 minutes for a 2.9 GB ISO in QEMU (TCG much slower than KVM; even with KVM,
virtual ATAPI throughput limits). Serial log shows only BdsDxe output (installer uses
`console=tty0`, not ttyS0). Use the Python VNC client to poll framebuffer; ~98% dark +
~2% gray ncurses text confirms Anaconda TUI is up.