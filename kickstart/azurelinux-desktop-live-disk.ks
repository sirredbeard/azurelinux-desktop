...
# Exact-kernel USB HID + mass-storage kmods (Pages repo). Same as live ISO.
...
# Couples kernel to desktop OOT kmods (USB, Wi-Fi, sound, BT, UVC, thinkpad, typec).
...
# build if any required asset is missing (warnings used to ship images
...
# Keep Plymouth animated for the full boot duration. Without this,
# plymouth-quit-wait.service fires after basic.target on fast/VM boots
# (well under a second) - the animated dots flash and the static logo
# takes over before systemd services are up. After=multi-user.target
# holds the splash alive until the full service graph is ready.
...
# Install Copilot GUI/CLI and microsoft/edit from %post --nochroot staging.
...
# First-boot appstream refresh if %post could not pull it (offline).
